...
async function readQuestion(qNo) {
    ...
    if (qtype == "SA" || qtype == "LA") {
        ...
        ques = ques.replace("______", " dash dash ")
        addSynth += msg_sayWrite;
        ...
        addSynth += msg_sayHelp + msg_sayExamStatus;
...
async function navigateExam(qNo, navQus) {
    ...
    } else if (asr_out.includes("hightlight") && (qtype == "SA" || qtype == "LA")) {
...
async function emphasizeAnswer(qNo, speechResult) {
    ...
    var answer_to_emhasize = document.getElementById("answer" + qNo)
    var written_answer = answer_to_emhasize.innerHTML;
    if (written_answer.includes(asr_out)) {
        speechResult_bold = "<b>" + asr_out + "</b>"
        res = written_answer.replace(asr_out, speechResult_bold)
        answer_to_emhasize.innerHTML = res;
    } else {
        console.log(msg_sryNtFound + asr_out);
        return;
    }
}
...
//         logger.log({mode: 'system', type: 'violation', activity: 'Blocked key combination ' + violation_count});
//     } 
// });
