...
function handleResume(button) {
    ...
    // console.log("Resume debug:",lastPlayedQuestion, currentQno, audio.src)
    if (isRecording) {
        pauseResumeRecording();
    }else if (lastPlayedQuestion === currentQno && audio.src) {
...
async function getServiceId(task, lang, text, selected = true) {
    await $.ajax({
